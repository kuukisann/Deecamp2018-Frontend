...
(function (global) {
    ...
    var set_init_model = function (model_raw_data) {
        ...
        model_data_archive[
        init_model_data[0].name + "_" + init_model_data[0].mode
            ] = model_raw_data;
    };
    ...
    var change_model = function (
        ...
        if (
            ...
            if (model_data_archive[model_name]) {
                ...
                var new_model = new SketchRNN(
                    JSON.parse(model_data_archive[model_name])
                );
                if (call_back) {
                    call_back(new_model);
                }
...
LSTMCell.prototype.forward = function (x, h, c) {
    ...
    var f = nj.sigmoid(
        nj.add(hidden.slice([2 * num_units, 3 * num_units]), forget_bias)
    );
...
function SketchRNN(model_raw_data) {
    ...
    function load_model(model_raw_data) {
        ...
        for (i = 0; i < num_blobs; i++) {
            weights[i] = nj.array(
                new Float32Array(string_to_array(weightsIn[i])),
                "float32"
            );
        ...
        console.log(
            "scale factor=" + Math.round(1000 * info.scale_factor) / 1000
        );
    ...
    function encode(sequence, temperature) {
        ...
        for (i = 0; i < N; i++) {
            s = [
                sequence[i][0] / scale_factor,
                sequence[i][1] / scale_factor,
                sequence[i][2],
                sequence[i][3],
                sequence[i][4]
            ];
        ...
        for (i = N - 1; i >= 0; i--) {
            s = [
                forward_sequence[i][0],
                forward_sequence[i][1],
                forward_sequence[i][2],
                forward_sequence[i][3],
                forward_sequence[i][4]
            ];
        ...
        if (temp > 0) {
            ...
            var eps = nj.multiply(
                nj.array(random_normal_vector(), "float32"),
                temp
            );
    ...
    function get_init_state_from_latent_vector(y) {
        if (y.constructor != Array) {
            console.log(
                "error, the argument passed into decode must be a Javascript normal Array type."
            );
    ...
    function decode(y, temperature, softmax_temperature) {
        ...
        for (var i = 0; i < max_seq_len; i++) {
            ...
            [dx, dy, pen_down, pen_up, pen_end] = sample(
                pdf,
                temp,
                softmax_temp
            );
